...
class BookService {
    ...
    async getTopThreeBorrowers(req, res) {
        try {
            const books = await bookRepo.getTopThreeBorrowers();
            res.status(200).json(books);
        } catch (err) {
            res.status(500).send(err);
        }
    ...
    async getTopThreeBorroweredBooks(req, res) {
        try {
            const books = await bookRepo.getTopThreeBorroweredBooks();
            res.status(200).json(books);
        } catch (err) {
            res.status(500).send(err);
        }
    ...
    async getBorrowingSummary(req, res) {
        try {
            const books = await bookRepo.getBorrowingSummary();
            res.status(200).json(books);
        } catch (err) {
            res.status(500).send(err);
        }
